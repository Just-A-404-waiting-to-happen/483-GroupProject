...
class IRSystem:
    ...
    def run_query(self, query: str) -> List[Dict]:
        ...
        terms_process = query.lower().split()
        terms = []
        for term in terms_process:
        ...
        return [{
            ...
            'genre': self.metadata[doc_id]['genre'],
            'score': score
...
def main(corpus: str):
    ...
    try:
        ...
        test_df = pd.read_csv('songs_filtered_test.csv')
        # for each row in the dataframe, get the lyrics and put em into the ir system
        for index, row in test_df.iterrows():
            print(index)
            try:
                query = row['lyrics'].strip()        
                results = ir.run_query(query)
                genredict = collections.defaultdict(float)
                # time to do some weighted averages
                i = 10
                for result in results:
                    # we need to split the genre
                    genres = result['genre'].split(',')
                    for genre in genres:
                        # remove whitespace
                        genre = genre.strip()
                        # add the score to the genre
                        genredict[genre] += result['score'] * i
                    i -= 1
                # sort the genres by score
                sorted_genres = sorted(genredict.items(), key=lambda x: x[1], reverse=True)
                for test_genre in row['genre'].split(','):
                    print(test_genre)
                    top5_genres = [genre for genre, score in sorted_genres[:5]]
                    print(top5_genres)
                    if test_genre in top5_genres:
                        tp += 1
                total += 1
            except Exception as e:
                print(f"Error: {str(e)}")
